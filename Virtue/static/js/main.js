...
document.querySelectorAll('.proj-container .proj-special-btn').forEach(btn => {
  let lastX = 0, lastY = 0; // 每个按钮独立的状态变量
  ...
  btn.onmousemove = function (e) { 
    ...
    const clampedX = Math.min(Math.max(targetX, 0), rect.width);
    const clampedY = Math.min(Math.max(targetY, 0), rect.height);
    
    lastX = lastX + (clampedX - lastX) * smoothFactor;
    lastY = lastY + (clampedY - lastY) * smoothFactor;
    
    // 使用带前缀的CSS变量
    btn.style.setProperty("--proj-x", lastX + "px");
    btn.style.setProperty("--proj-y", lastY + "px");
}
...
btn.onmouseleave = function (e) {
    ...
    btn.style.setProperty("--proj-x", x + "px");
    btn.style.setProperty("--proj-y", y + "px");
    setTimeout(() => {
